...
final class Api {
    static final class ApiException extends IOException {
        ...
        boolean authenticationRequired(){return status==401||status==403;}
    }
    ...
    private static String raw(String method,String path,JSONObject body,String token)throws Exception{
        ...
        int status=c.getResponseCode();InputStream stream=status>=400?c.getErrorStream():c.getInputStream();String text=stream==null?"":new String(stream.readAllBytes(),StandardCharsets.UTF_8);if(status>=400){String message=text;try{message=new JSONObject(text).optString("message",text);}catch(JSONException ignored){}throw new ApiException(status,message);}return text;
    }
}

#!/usr/bin/env python3
"""Official Korean Farm Map Shapefile ZIP or .shp -> PostGIS importer."""
...
def datasets(source):
    """Yield (reader, projection WKT) pairs from an archive or shapefile set."""
    if zipfile.is_zipfile(source):
        with zipfile.ZipFile(source) as archive:
            for files in dataset_members(archive):
                prj=archive.read(files[".prj"]).decode("utf-8",errors="replace")
                encoding="euc-kr"
                if ".cpg" in files:
                    encoding=archive.read(files[".cpg"]).decode("ascii",errors="ignore").strip() or encoding
                yield shapefile.Reader(
                    shp=io.BytesIO(archive.read(files[".shp"])),
                    shx=io.BytesIO(archive.read(files[".shx"])),
                    dbf=io.BytesIO(archive.read(files[".dbf"])), encoding=encoding),prj
        return
    shp=Path(source)
    if shp.suffix.lower() != ".shp":
        raise ValueError("입력 파일은 팜맵 ZIP 또는 .shp여야 합니다.")
    missing=[str(shp.with_suffix(x)) for x in (".shx",".dbf",".prj") if not shp.with_suffix(x).exists()]
    if missing:
        raise FileNotFoundError("필수 Shapefile 구성 파일이 없습니다: "+", ".join(missing))
    encoding="euc-kr"
    cpg=shp.with_suffix(".cpg")
    if cpg.exists(): encoding=cpg.read_text(errors="ignore").strip() or encoding
    reader=shapefile.Reader(str(shp),encoding=encoding)
    try: yield reader,shp.with_suffix(".prj").read_text(errors="replace")
    finally: reader.close()

def main():
    p=argparse.ArgumentParser();p.add_argument("source",help="팜맵 ZIP 또는 .shp 파일");p.add_argument("--district");p.add_argument("--locality");p.add_argument("--batch-size",type=int,default=500);args=p.parse_args()
    ...
    with psycopg.connect(connection_url) as conn:
      for reader,prj in datasets(args.source):

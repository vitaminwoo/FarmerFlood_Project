...
ROOT = Path(os.getenv("FLOOD_ROOT", Path(__file__).resolve().parents[1])).resolve()
...
FONT_PATH = Path(os.getenv("FLOOD_FONT_PATH", "/System/Library/Fonts/AppleSDGothicNeo.ttc"))
PROGRESS_PATH = Path(os.getenv("FLOOD_PROGRESS_FILE", OUT / "progress.json"))
DT_CACHE_DIR = Path(os.environ["FLOOD_DT_CACHE_DIR"]).resolve() if os.getenv("FLOOD_DT_CACHE_DIR") else None
FPS = 24
...
def publish_progress(stage: str, detail: Dict[str, Any] | None = None) -> None:
    ...
    PROGRESS_PATH.write_text(json.dumps({"status": "RUNNING", "stage": stage, "detail": payload}, ensure_ascii=False), encoding="utf-8")
    print(f"{datetime.now(timezone.utc).isoformat()} [{stage}] {json.dumps(payload, ensure_ascii=False)}", flush=True)
...
def safety_agent(state: StoryState) -> Dict[str, Any]:
    ...
    rain = state["input_data"]["forecast"]["predicted_24h_rain_mm"]
    scenario = state["field_result"]["scenario_results"]
    warning = scenario["warning"]["inundated_percent"]
    serious = scenario["serious"]["inundated_percent"]
    required = [f"{rain:g}", f"{warning:.1f}", f"{serious:.1f}", "배수로", "농기계", "비닐하우스", "논둑이나 물꼬", "일일구"]
...
def tts_agent(state: StoryState) -> Dict[str, Any]:
    publish_progress("tts_agent_running")
    ...
    for index, item in enumerate(state["segments"], start=1):
        ...
        try:
            ...
            for _ in range(3):
                if duration <= target:
                    break
                next_speed = min(2.0, round(speed * duration / target * 1.04, 2))
                if next_speed <= speed:
                    break
                speed = next_speed
                write_openai_audio(client, model, voice, item["narration"], speed, path)
            ...
            if duration > target:
                raise RuntimeError(f"OpenAI TTS duration exceeds segment after adjustment: {duration:.3f}/{target:.3f}")
...
def base_render_agent(state: StoryState) -> Dict[str, Any]:
    publish_progress("base_render_agent_running")
    log = OUT / "base_render.log"
    report_path = OUT / "base_render_report.json"
    if DT_CACHE_DIR:
        cached_video = DT_CACHE_DIR / "digital-twin-base.mp4"
        cached_report = DT_CACHE_DIR / "base-render-report.json"
        if cached_video.is_file() and cached_video.stat().st_size >= 100_000 and cached_report.is_file():
            OUT.mkdir(parents=True, exist_ok=True)
            BASE_VIDEO.unlink(missing_ok=True)
            BASE_VIDEO.symlink_to(cached_video)
            report_path.unlink(missing_ok=True)
            report_path.symlink_to(cached_report)
            log.write_text("[CACHE HIT] 동일 디지털 트윈 입력의 합성 전 원본 MP4를 재사용했습니다. 최종 개인화 영상은 새로 제작합니다.\n", encoding="utf-8")
            publish_progress("base_render_agent", {"base_video_bytes": BASE_VIDEO.stat().st_size, "reused": True, "cache": "HIT"})
            return {"trace": traced(state, "base_render_agent", {"base_video_bytes": BASE_VIDEO.stat().st_size, "reused": True, "cache": "HIT"})}
    ...
    with log.open("w", encoding="utf-8") as output:
        for line in process.stdout or []:
            ...
            if match:
                publish_progress("base_render_agent_running", {"frame": int(match.group(1)), "total_frames": 1248, "cache": "MISS"})
    returncode = process.wait()
    if returncode != 0 or not BASE_VIDEO.is_file() or BASE_VIDEO.stat().st_size < 100_000:
        raise RuntimeError(f"base render failed: {log}")
    if DT_CACHE_DIR:
        DT_CACHE_DIR.mkdir(parents=True, exist_ok=True)
        shutil.copy2(BASE_VIDEO, DT_CACHE_DIR / "digital-twin-base.mp4")
        shutil.copy2(report_path, DT_CACHE_DIR / "base-render-report.json")
        rendered_scene = ROOT / "blender" / "gangnae_story_v9_base.blend"
        if rendered_scene.is_file():
            shutil.copy2(rendered_scene, DT_CACHE_DIR / "scene.blend")
            rendered_scene.unlink()
        BASE_VIDEO.unlink()
        BASE_VIDEO.symlink_to(DT_CACHE_DIR / "digital-twin-base.mp4")
    return {"trace": traced(state, "base_render_agent", {"base_video_bytes": BASE_VIDEO.stat().st_size, "log": str(log), "reused": False, "cache": "MISS"})}
...
def main():
    ...
    result = graph.invoke({"run_id": os.getenv("FLOOD_RUN_ID", "GANGNAE-GUIDANCE-V9-60S"), "trace": []})
    print(json.dumps({
